...
import static com.orane.icliniq.fileattach_library.EasyImageFiles.singleFileList;
...
public class EasyImage implements Constants {
    ...
    private static Intent createDocumentsIntent(@NonNull Context context, int type) {
        ...
        //intent.setType("*/*");
        intent.setType("*/*");
        String[] extraMimeTypes = {"application/pdf", "application/doc", "image/jpg","image/jpeg", "image/png","image/gif" };
...
/*        intent.setType("application/pdf");
    ...
    public static void openDocuments2(Activity activity, int type) {
        Intent intent = createDocumentsIntent(activity, type);
        activity.startActivityForResult(intent, RequestCodes.PICK_PICTURE_FROM_DOCUMENTS);
    }
